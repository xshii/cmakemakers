...
import { CMakeProject, Target, Scope, SourceEntry, CMakeGenerationResult } from '../model/types';
import {
...
export class CMakeGenerator {
  ...
  /**
   * Generate multiple CMakeLists.txt files with smart directory placement.
   *
   * Strategy:
   * 1. Group targets by their source file directories
   * 2. Detect intermediate directories (3+ subdirectories)
   * 3. Generate main CMakeLists.txt with project() and global config
   * 4. Generate sub CMakeLists.txt for each target group
   * 5. Generate intermediate CMakeLists.txt for organization
   *
   * @param project The CMake project configuration
   * @param projectRoot The absolute path to project root directory
   * @returns Generation result with all file paths and contents
   */
  generateMultiple(project: CMakeProject, projectRoot: string): CMakeGenerationResult {
    const files = new Map<string, string>();

    // Step 1: Group targets by directory
    const targetGroups = groupTargetsByDirectory(project.targets, projectRoot);

    // Step 2: Detect intermediate directories
    const intermediates = detectIntermediateDirectories(targetGroups, projectRoot);

    // Step 3: Generate CMakeLists.txt for each target group
    for (const [dir, targets] of targetGroups) {
      const content = this.generateSubCMakeLists(targets, dir, projectRoot);
      const filePath = path.join(dir, 'CMakeLists.txt');
      files.set(filePath, content);
    }

    // Step 4: Generate intermediate CMakeLists.txt files
    for (const [dir, subdirs] of intermediates) {
      const content = this.generateIntermediateCMakeLists(subdirs, dir, projectRoot);
      const filePath = path.join(dir, 'CMakeLists.txt');
      files.set(filePath, content);
    }

    // Step 5: Generate main CMakeLists.txt
    const mainFilePath = path.join(projectRoot, 'CMakeLists.txt');
    const mainContent = this.generateMainCMakeLists(
      project,
      targetGroups,
      intermediates,
      projectRoot
    );
    files.set(mainFilePath, mainContent);
  ...
  /**
   * Generate main CMakeLists.txt with project() declaration and global config.
   * Includes add_subdirectory() calls for all subprojects.
   */
  private generateMainCMakeLists(
    ...
    // Global configuration
    lines.push(...this.generateGlobalConfig(project));
    ...
    // Global find_package dependencies
    if (project.dependencies && project.dependencies.length > 0) {
      lines.push('# Dependencies');
      for (const dep of project.dependencies) {
        if (dep.type === 'find_package' && dep.find_package) {
    ...
    // Collect all directories to include
    const dirsToInclude = new Set<string>();

    // Add intermediate directories first
    for (const dir of intermediates.keys()) {
      dirsToInclude.add(dir);
    }

    // Add target group directories that aren't already included via intermediates
    for (const dir of targetGroups.keys()) {
      if (dir === projectRoot) {
        continue; // Skip root directory
      }

      // Check if this directory is a child of any intermediate
      let isChildOfIntermediate = false;
      for (const intermediateDir of intermediates.keys()) {
        if (dir.startsWith(intermediateDir + path.sep)) {
          isChildOfIntermediate = true;
          break;
        }
      }

      if (!isChildOfIntermediate) {
        dirsToInclude.add(dir);
      }
    }

    // Generate add_subdirectory() for each directory
    const sortedDirs = Array.from(dirsToInclude).sort();
    for (const dir of sortedDirs) {
      const relPath = path.relative(projectRoot, dir);
      if (relPath) {
        const cmakePath = toCMakePath(relPath);
        lines.push(`add_subdirectory(${cmakePath})`);
      }
    }
  ...
  /**
   * Generate CMakeLists.txt for a group of targets in a subdirectory.
   * Does not include project() declaration - only target definitions.
   */
  private generateSubCMakeLists(targets: Target[], directory: string, projectRoot: string): string {
    const lines: string[] = [];
  ...
  /**
   * Generate intermediate CMakeLists.txt that only contains add_subdirectory() calls.
   * Used when a parent directory has multiple child directories with targets (3+).
   */
  private generateIntermediateCMakeLists(
    ...
    lines.push('# Generated by CMakeMakers');
    lines.push('# Intermediate CMakeLists.txt for directory organization');
    lines.push('');

    for (const subdir of subdirectories.sort()) {
      const relPath = path.relative(directory, subdir);
      if (relPath) {
        const cmakePath = toCMakePath(relPath);
        lines.push(`add_subdirectory(${cmakePath})`);
      }
  ...
  /**
   * Extract global configuration generation logic.
   * Refactored from the original generate() method.
   */
  private generateGlobalConfig(project: CMakeProject): string[] {
    const lines: string[] = [];
